	    ...
	    if (localStorage != null) { return localStorage.getItem(key); }
         ...
         function PopulateVendoOptions() {
            $.get("nodes.txt", function(data, status){
               ...
               data = '['+ trimTrailingChars(data, ',') +']'; // fix json string
               //console.log(data);
...
function initVoucherHandler() {
      var timer = setInterval(function() {
        if (AjaxEnable && (!TimedOut)) {
         ...
         var RateTypeSelectedText = $('#RateType').find(":selected").text(); // console.log(RateTypeSelected);
         $('#RateToUse').text(RateTypeSelectedText);
         $('#ProgressBar').show();
         ...
         currentRequest = jQuery.ajax({
            ...
            success: function(data) {
               ...
               else if (Client.Status=="TERMINATED1" || Client.Status=="TERMINATED2" || Client.Status=="TERMINATED3") {
                  TimedOut=true;
                  $('#ProgressBar').hide();
                  $('#InsertCoinMessage').html(Client.Message); 
               }
               else {
                  TimedOut=true;
...
/**/
         ...
         $(document).on('click','#RatesButton',function(e){
            var RateTypeSelected = $('#RateType').find(":selected").val(); // console.log(RateTypeSelected);
            var RateTypeSelectedText = $('#RateType').find(":selected").text(); // console.log(RateTypeSelected);
            var VendoSelectedText = $('#VendoSelection').find(":selected").text();
            //console.log(RateTypeSelected);
            $('#RatesTable').text(RateTypeSelectedText + " - " + VendoSelectedText);

            $('#RatesModalBody').html("Requesting from vendo, please wait...");
            var Req = $.ajax({
               url: Server+"getRates.php",
               type: 'post',
               data: {'RateType':RateTypeSelected },
               success: function(data) {
         ...
         $(document).on('click','#SavedVoucherButton',function(e){
            ShowVouchers();
         });
